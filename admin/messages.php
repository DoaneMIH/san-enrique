    ...
    <div class="row g-4">
      ...
      <div class="col-lg-<?= $viewMsg ? '5' : '12' ?>">
        <div class="admin-table-wrap">
                ...
                <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:0.5rem;">
                  <div style="flex:1;min-width:0;">
                    <div style="display:flex;align-items:center;gap:6px;">
                      <div style="width:30px;height:30px;background:linear-gradient(135deg,var(--primary),var(--accent));border-radius:8px;display:flex;align-items:center;justify-content:center;color:white;font-size:0.75rem;font-weight:700;flex-shrink:0;">
                        <?= strtoupper(substr($msg['name'], 0, 1)) ?>
                      </div>
                      <div style="min-width:0;">
                        <div style="font-weight:<?= $isUnread ? '700' : '600' ?>;font-size:0.86rem;color:var(--primary);display:flex;align-items:center;gap:5px;">
                          <?= htmlspecialchars($msg['name']) ?>
                          <?php if ($isUnread): ?><span style="width:7px;height:7px;background:var(--accent);border-radius:50%;display:inline-block;flex-shrink:0;"></span><?php endif; ?>
                        </div>
                        <div style="font-size:0.75rem;color:var(--text-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><?= htmlspecialchars($msg['email']) ?></div>
                      </div>
                    </div>
                    <div style="font-size:0.82rem;color:var(--primary-mid);margin-top:5px;font-weight:600;"><?= htmlspecialchars(substr($msg['subject'], 0, 45)) ?><?= strlen($msg['subject']) > 45 ? '...' : '' ?></div>
                    <div style="font-size:0.78rem;color:var(--text-muted);margin-top:2px;"><?= htmlspecialchars(substr($msg['message'], 0, 60)) ?>...</div>
                  </div>
                  <div style="text-align:right;flex-shrink:0;display:flex;flex-direction:column;align-items:flex-end;gap:4px;">
                    <div style="font-size:0.7rem;color:var(--gray-500);white-space:nowrap;"><?= date('M j', strtotime($msg['created_at'])) ?></div>
          ...
          <div style="text-align:center;padding:4rem 2rem;color:var(--text-muted);">
            <i class="fas fa-inbox" style="font-size:3.5rem;color:var(--gray-200);display:block;margin-bottom:1rem;"></i>
      ...
      <div class="col-lg-7">
        <div class="admin-form-card" style="height:100%;">
          <div class="admin-form-header" style="display:flex;align-items:flex-start;justify-content:space-between;gap:1rem;">
            <div>
              <div style="font-family:'Playfair Display',serif;font-size:1.05rem;color:var(--primary);margin-bottom:3px;">
          ...
          <div class="admin-form-body">
            <!-- Sender Info Card -->
            <div style="display:flex;gap:14px;align-items:center;padding:1.1rem 1.25rem;background:var(--content-bg);border-radius:12px;margin-bottom:1.5rem;border:1px solid var(--border);">
              <div style="width:50px;height:50px;background:linear-gradient(135deg,var(--primary),var(--accent));border-radius:14px;display:flex;align-items:center;justify-content:center;color:white;font-weight:700;font-size:1.3rem;flex-shrink:0;">
                <?= strtoupper(substr($viewMsg['name'], 0, 1)) ?>
              </div>
              <div>
                <div style="font-weight:700;color:var(--primary);font-size:1rem;"><?= htmlspecialchars($viewMsg['name']) ?></div>
                <div style="font-size:0.83rem;color:var(--text-muted);margin-top:1px;"><?= htmlspecialchars($viewMsg['email']) ?></div>
              </div>
            ...
            <!-- Message Body -->
            <div style="background:white;border:1px solid var(--border);border-radius:12px;padding:1.5rem;line-height:1.9;color:var(--text);font-size:0.92rem;white-space:pre-wrap;min-height:180px;">
              <?= htmlspecialchars($viewMsg['message']) ?>
            </div>

            <!-- Reply Button -->
            <div style="margin-top:1.5rem;padding-top:1.25rem;border-top:1px solid var(--border);display:flex;gap:0.75rem;flex-wrap:wrap;">
