...
<div class="admin-content">
  <div class="admin-topbar">
    <div>
      ...
      <div class="topbar-breadcrumb"><?= $totalReviews ?> total reviews · Avg <?= number_format($avgRating, 1) ?>★</div>
    </div>
  </div>
    ...
    <div class="row g-3 mb-4">
      <div class="col-6 col-md-3">
        <div class="dash-stat-card">
          <div class="stat-icon" style="background:linear-gradient(135deg,#d4a017,#f0c040);"><i class="fas fa-star"></i></div>
      ...
      <div class="col-6 col-md-3">
        <div class="dash-stat-card">
          <div class="stat-icon" style="background:linear-gradient(135deg,#2d6a4f,#52b788);"><i class="fas fa-trophy"></i></div>
      ...
      <div class="col-6 col-md-3">
        <div class="dash-stat-card">
          <div class="stat-icon" style="background:linear-gradient(135deg,#1b6fb0,#3b9dd1);"><i class="fas fa-thumbs-up"></i></div>
      ...
      <div class="col-6 col-md-3">
        <div class="dash-stat-card">
          <div class="stat-icon" style="background:linear-gradient(135deg,#c0392b,#e74c3c);"><i class="fas fa-thumbs-down"></i></div>
            ...
            <tr>
              ...
              <td>
                <div style="display:flex;align-items:center;gap:8px;">
                  <div style="width:32px;height:32px;border-radius:8px;background:linear-gradient(135deg,var(--primary),var(--accent));display:flex;align-items:center;justify-content:center;color:white;font-weight:700;font-size:0.8rem;flex-shrink:0;">
                    <?= strtoupper(substr($review['reviewer_name'] ?: 'A', 0, 1)) ?>
                  </div>
                  <span style="font-weight:600;font-size:0.87rem;color:var(--primary);"><?= htmlspecialchars($review['reviewer_name'] ?: 'Anonymous') ?></span>
                </div>
              </td>
              <td>
                <a href="../listing.php?slug=<?= urlencode($review['listing_slug']) ?>" target="_blank"
                  ...
                  <?= htmlspecialchars($review['listing_name']) ?> <i class="fas fa-external-link-alt" style="font-size:0.65rem;"></i>
                </a>
              </td>
              <td>
              ...
              <td style="max-width:240px;">
                <div style="font-size:0.83rem;color:var(--text-muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="<?= htmlspecialchars($review['comment']) ?>">
                  <?= htmlspecialchars($review['comment']) ?>
                </div>
              </td>
              <td style="font-size:0.8rem;color:var(--text-muted);white-space:nowrap;"><?= date('M j, Y', strtotime($review['created_at'])) ?></td>
              <td>
                <button onclick="confirmDeleteReview(<?= $review['id'] ?>, '<?= addslashes($review['reviewer_name'] ?: 'Anonymous') ?>')"
