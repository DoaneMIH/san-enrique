...
if (!$slug) { header('Location: listings.php'); exit; }
...
if (!$result || $result->num_rows === 0) { header('Location: listings.php'); exit; }
...
.lv-status-badge.active   { background: rgba(82,183,136,0.2); color: #2d6a4f; }
.lv-status-badge.inactive { background: rgba(220,53,69,0.12); color: #c0392b; }
.lv-status-badge.draft    { background: rgba(255,193,7,0.18);  color: #856404; }
...
@media (max-width: 576px) { .lv-info-grid { grid-template-columns: 1fr; } }
.lv-info-item { display: flex; flex-direction: column; gap: 2px; }
.lv-info-label {
  font-size: 0.72rem;
  font-weight: 700;
...
.lv-info-value a { color: var(--accent, #52b788); }
...
.lv-reviewer { font-weight: 700; font-size: 0.88rem; color: var(--primary, #1b4332); }
.lv-stars    { font-size: 0.82rem; color: #d4a017; }
.lv-review-text { font-size: 0.84rem; color: var(--text-muted); margin: 0; }
.lv-review-date { font-size: 0.72rem; color: var(--gray-500, #adb5bd); margin-top: 0.3rem; }
  ...
  <div class="admin-main">
    ...
    <div class="row g-4">
      ...
      <div class="col-lg-8">
        ...
        <div class="lv-card">
          <div class="lv-card-title d-flex justify-content-between align-items-center" style="border:none;padding-bottom:0.75rem;">
          ...
          <div class="lv-map"><div id="lvMap"></div></div>
          <div style="font-size:0.77rem;color:var(--text-muted);margin-top:0.5rem;">
            Coordinates: <?= number_format((float)$listing['latitude'], 6) ?>, <?= number_format((float)$listing['longitude'], 6) ?>
          </div>
        </div>
        <?php endif; ?>
        ...
        <div class="lv-card">
          <div class="lv-card-title">
            ...
            <span style="margin-left:auto;font-size:0.82rem;color:var(--text-muted);font-weight:500;"><?= count($reviews) ?> total</span>
          </div>
          <?php if ($reviews): ?>
            <?php
            $avg = round(array_sum(array_column($reviews, 'rating')) / count($reviews), 1);
            ?>
            <div style="display:flex;align-items:center;gap:1rem;margin-bottom:1rem;padding:0.75rem 1rem;background:var(--sidebar-bg,#f7faf7);border-radius:10px;">
              <div style="font-size:2rem;font-weight:800;color:var(--primary);line-height:1;"><?= $avg ?></div>
              <div>
                <div style="color:#d4a017;font-size:1rem;"><?= str_repeat('★', round($avg)) . str_repeat('☆', 5 - round($avg)) ?></div>
                <div style="font-size:0.78rem;color:var(--text-muted);">Average of <?= count($reviews) ?> review<?= count($reviews) !== 1 ? 's' : '' ?></div>
              </div>
            </div>
            ...
            <div class="lv-review">
              <div class="lv-review-header">
                ...
                <span class="lv-stars"><?= str_repeat('★', $review['rating']) . str_repeat('☆', 5 - $review['rating']) ?></span>
              </div>
        ...
        <div class="lv-card" style="position:sticky;top:80px;">
          ...
          <div class="lv-info-grid">
            ...
            <div class="lv-info-item">
              ...
              <span class="lv-info-value">
                <i class="<?= htmlspecialchars($listing['icon']) ?> me-1" style="color:<?= htmlspecialchars($listing['color']) ?>;"></i>
                <?= htmlspecialchars($listing['category_name']) ?>
              </span>
            </div>
            ...
            <div class="lv-info-item" style="grid-column:1/-1;">
              ...
              <span class="lv-info-value" style="font-family:monospace;font-size:0.82rem;"><?= htmlspecialchars($listing['slug']) ?></span>
            </div>
            <?php endif; ?>
            ...
            <div class="lv-info-item" style="grid-column:1/-1;">
              ...
              <span class="lv-info-value">
                <a href="mailto:<?= htmlspecialchars($listing['email']) ?>"><?= htmlspecialchars($listing['email']) ?></a>
              </span>
            </div>
            <?php endif; ?>
          ...
          <!-- Quick Actions -->
          <div style="margin-top:1.25rem;padding-top:1rem;border-top:1px solid var(--border,#e8ede8);display:flex;flex-direction:column;gap:0.5rem;">
            <a href="listings.php?action=edit&id=<?= $listing['id'] ?>" class="btn-admin-primary" style="justify-content:center;text-align:center;width:100%;">
              <i class="fas fa-pencil-alt me-2"></i>Edit This Listing
            </a>
            <a href="../listing.php?slug=<?= urlencode($listing['slug']) ?>" target="_blank" class="btn-admin-secondary" style="justify-content:center;text-align:center;width:100%;">
...
<script src="https://maps.googleapis.com/maps/api/js?key=<?= GOOGLE_MAPS_API_KEY ?>&callback=initAdminMap" defer></script>
<?php endif; ?>
</body>
</html>
