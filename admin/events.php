...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ── Rebuild image list from scratch using only the kept[] inputs JS sends ──
    // JS removes a thumb → removes it from kept[], appends its path to remove_evt_img[].
    // PHP never touches the old JSON blob; it just uses what JS explicitly kept.
    $evtImgPaths = [];
    $keptRaw = $_POST['kept_evt_img'] ?? [];
    if (!is_array($keptRaw)) $keptRaw = [$keptRaw];
    foreach ($keptRaw as $kp) {
        $kp = trim($kp);
        if ($kp !== '') $evtImgPaths[] = $kp;
    }

    // Delete physical files for any removed local uploads
    $toRemove = $_POST['remove_evt_img'] ?? [];
    if (!is_array($toRemove)) $toRemove = [$toRemove];
    foreach ($toRemove as $rp) {
        $rp = trim($rp);
        if ($rp === '') continue;
        if (strpos($rp, '../uploads/events/') === 0) {
            if (file_exists($rp)) unlink($rp);
            $abs = realpath(__DIR__ . '/' . $rp);
    ...
    // NOTE: The `image` column must be TEXT or LONGTEXT in MySQL.
    // If you see "Data too long" errors, run: ALTER TABLE events MODIFY COLUMN image TEXT;
    $evtImgPaths = array_values(array_filter($evtImgPaths)); // remove nulls/empty
    $imageJson   = json_encode($evtImgPaths, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
        ...
        <div class="admin-form-body">
          <div class="row g-3">
            ...
            <div class="col-12">
              ...
              ?>
              <!-- One hidden input per kept image; JS removes them when admin clicks X -->
              <div id="evtKeptInputs">
<?php foreach ($evtExisting as $kIdx => $kImg): ?>
                <input type="hidden" name="kept_evt_img[]" value="<?= htmlspecialchars($kImg) ?>" data-idx="<?= $kIdx ?>">
<?php endforeach; ?>
              </div>
              ...
              <div id="evtExistingWrap" class="gallery-existing-wrap">
                ...
                ?>
                <div class="gallery-thumb-wrap<?= $gi === 0 ? ' evt-thumb-first' : '' ?>" data-idx="<?= $gi ?>">
                  <img src="<?= htmlspecialchars($gDisp) ?>" onerror="this.src='https://placehold.co/100x80/1b4332/fff?text=IMG'">
                  <button type="button" onclick="evtRemoveExisting(this,<?= $gi ?>,'<?= addslashes($gImg) ?>')" class="gallery-thumb-remove" title="Remove">
...
function evtRemoveExisting(btn, idx, path) {
  var wrap = btn.closest('.gallery-thumb-wrap');

  // Remove the kept_evt_img[] hidden input that matches this index
  var keptContainer = document.getElementById('evtKeptInputs');
  if (keptContainer) {
    var match = keptContainer.querySelector('input[data-idx="' + idx + '"]');
    if (match) match.remove();
  }

  // Track for physical file deletion on server (local uploads only)
  var rem = document.createElement('input');
  rem.type = 'hidden'; rem.name = 'remove_evt_img[]'; rem.value = path;
  document.getElementById('evtRemovedInputs').appendChild(rem);
