                    ...
                    <tr>
                      <td>
                        <div style="display:flex;align-items:center;gap:10px;">
                          <img
                            src="<?= $listing['featured_image'] ?: 'https://placehold.co/48x38/1b4332/ffffff?text=?' ?>"
                          ...
                          <div>
                            ...
                            <div style="font-size:0.75rem;color:var(--text-muted);">
                              <?= htmlspecialchars($listing['barangay'] ?: 'N/A') ?></div>
                          </div>
                        </div>
                      </td>
                      <td><span
                      ...
                      <td><span class="status-badge <?= $listing['status'] ?>"><?= ucfirst($listing['status']) ?></span>
                      </td>
            ...
            <div style="padding:0.5rem;">
              <?php if ($recentMessages): ?>
                <?php foreach ($recentMessages as $msg): ?>
                  <div
                    style="padding:0.85rem;border-radius:10px;margin-bottom:4px;background:<?= $msg['is_read'] ? 'transparent' : 'var(--content-bg)' ?>;border:1px solid var(--border);transition:var(--transition);">
                    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:0.5rem;">
                      <div>
                        <div style="font-weight:700;font-size:0.87rem;color:var(--primary);">
                          <?= htmlspecialchars($msg['name']) ?></div>
                        <div style="font-size:0.78rem;color:var(--text-muted);margin-top:2px;">
                          <?= htmlspecialchars(substr($msg['subject'], 0, 35)) ?>...</div>
                      </div>
                      <?php if (!$msg['is_read']): ?>
                        <span
                          style="background:var(--accent);width:8px;height:8px;border-radius:50%;flex-shrink:0;margin-top:4px;"></span>
                      <?php endif; ?>
                    </div>
                    <div style="font-size:0.72rem;color:var(--gray-500);margin-top:4px;">
                      <?= date('M j, Y', strtotime($msg['created_at'])) ?></div>
                  </div>
                <?php endforeach; ?>
              <?php else: ?>
                <div style="text-align:center;padding:2rem;color:var(--text-muted);font-size:0.87rem;">
                  <i class="fas fa-inbox"
                    style="font-size:2rem;color:var(--gray-200);display:block;margin-bottom:0.5rem;"></i>
                  No messages yet
                </div>
              <?php endif; ?>
            </div>
